...
test("vault plugins are settings-gated and run commands through safe host paths", () => {
  ...
  assert.match(appTypes, /export type SettingsTab = "main" \| "appearance" \| "plugins" \| "ai" \| "debug"/);
...
test("AI commands use vault settings and review output before editing", () => {
  ...
  assert.match(app, /commandPaletteScope === "ai"[\s\S]*aiCommandPaletteCommands[\s\S]*commandPaletteScope === "insert"[\s\S]*insertCommandPaletteCommands[\s\S]*commandPaletteScope === "table"[\s\S]*tableCommandPaletteCommands[\s\S]*commandPaletteCommands/);
